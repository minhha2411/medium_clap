...
const withClapAnimation = (WrappedComponent) => {
  ...
  class WithClapAnimation extends React.Component {
    ...
    componentDidMount() {
      ...
      const clapEl = document.querySelector("#clap");
    ...
    render() {
      return (
        <WrappedComponent
          {...this.props}
          animation={this.state.animationTimeLine}
        />
      );
    }
  }
...
function Clap({ animation }) {
  ...
  const handleClickCLap = () => {
    ...
    setClapState((prevState) => ({
      ...prevState,
      count: Math.min(prevState.count + 1, MAXIMUM_COUNT),
  ...
  return (
    <button className={styles.clap} onClick={handleClickCLap} id="clap">
